...
public class NccDAO {
	...
	public void deletencc(String id) {
		String sql ="delete from ProductDATABASE.supplier where supplierId = ? ";
		try {
	...
	public void insertncc(String name, String phone, String address) {
		String sql ="INSERT INTO `ProductDATABASE`.`supplier` (`companyName`, `phone`, `address`) VALUES ( ?,?,?)";
		try {
			conn = DBConnect.getConnect();
			ps = conn.prepareStatement(sql);
	...
	public void updatencc(String nid, String name, String phone, String address) {
		String sql ="UPDATE `ProductDATABASE`.`supplier` SET `companyName` = ?, `phone` = ?, `address` = ? WHERE (`supplierId` = ?);";
		try {
			conn = DBConnect.getConnect();
			ps = conn.prepareStatement(sql);
	...
	//Hiển thị NCC theo ID 
		public NCCModel getNCCById(String id) {
			...
			String sql = "SELECT * FROM ProductDATABASE.supplier where supplierId=?;";
			try {
				new DBConnect();
				//Mở kết nối database
				conn = DBConnect.getConnect();
