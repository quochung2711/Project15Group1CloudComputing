...
public class NewsDAO {
	...
	public void insertnew(String image, String title, String brief, String content) {
		String sql ="INSERT INTO `ProductDATABASE`.`News` (`imageNews`, `title`, `brief`, `content`) VALUES ( ?,?,?,?)";
		try {
			conn = DBConnect.getConnect();
			ps = conn.prepareStatement(sql);
	...
	public void updatenew(String nid, String image, String title, String brief, String content) {
		String sql ="UPDATE `ProductDATABASE`.`News` SET `imageNews` = ?, `title` = ?, `brief` = ?, `content` = ? WHERE (`id` = ?)";
		try {
			conn = DBConnect.getConnect();
			ps = conn.prepareStatement(sql);
	...
	public NewsModel getNewsById(String id) {
		...
		String sql = "SELECT * FROM ProductDATABASE.News where id = ?";
		try {
			new DBConnect();
			//Mở kết nối database
			conn = DBConnect.getConnect();
