...
try{
	...
	ResultSet resultSet = statement.executeQuery("select * from theothang");
	int i =0;
	while(resultSet.next()){
		map = new HashMap<Object,Object>(); 
		map.put("label", resultSet.getString(2)); 
		map.put("y", resultSet.getInt(1)); 
		list.add(map);
...
<body>
	...
	<c:forEach var="income" items="${listincome }" >
				<p>Thang bán: ${income.date }</p>
				<br>
				<p>Tổng số tiền thu được: ${income.doanhthu }</p>
			</c:forEach>
	</div>
			<!-- /row -->
		</div>
