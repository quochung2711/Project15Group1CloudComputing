...
public class ProductDAO {
	...
	public ProductModel getbyid(int id)
	{
		...
		String sql = "SELECT * FROM ProductDATABASE.product where productId=?;";
		try {
			new DBConnect();
			...
			while(rs.next()) {
				...
				String name = rs.getString("namesp");
	...
	public void insert(ProductModel productmodel){
		//Khai báo list để lưu danh sách danh mục
		  ...
		  String sql = "INSERT INTO `ProductDATABASE`.`product` (`catalogId`, `supplierId`, `namesp`, `price`, `content`, `discount`, `image_link`, `amount`, `sl`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	public void delete(int id) {
		String sql ="delete FROM ProductDATABASE.product where productId=?;";
		try {
			conn = DBConnect.getConnect();
			ps = conn.prepareStatement(sql);
			ps.setInt(1, id);
	...
	public void edit(ProductModel productmodel) {
		String sql ="UPDATE `ProductDATABASE`.`product` SET `catalogId` = ?, `supplierId` = ?, `namesp` =?, `price` = ?, `content` = ?, `discount` = ?, `image_link` = ?, `amount` = ?, `sl` = ? WHERE (`productId` = ?);";
		try {
			conn = DBConnect.getConnect();
			ps = conn.prepareStatement(sql);
			...
			ps.setInt(8, productmodel.getAmount());
			ps.setInt(9, productmodel.getSl());
			ps.setInt(10, productmodel.getpID());
			ps.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<ProductModel> getAllProduct(){
		//Khai bÃ¡o list Ä‘á»ƒ lÆ°u danh sÃ¡ch sáº£n pháº©m
				...
				//Khi bÃ¡o chuá»—i truy váº¥n
				String sql = "SELECT * FROM ProductDATABASE.product";
	...
	public List<ProductModel> getTop5BestSeller(){
		//Khai bÃ¡o list Ä‘á»ƒ lÆ°u danh sÃ¡ch sáº£n pháº©m
				...
				//Khi bÃ¡o chuá»—i truy váº¥n
				String sql = "SELECT * FROM ProductDATABASE.Show_Top6_product;";
	...
	public List<ProductModel> getTop6Product(){
		...
		//Khi bÃ¡o chuá»—i truy váº¥n
		String sql = "SELECT * FROM ProductDATABASE.show_Top6_newProduct";
